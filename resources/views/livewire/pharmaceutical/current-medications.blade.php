            ...
            @forelse($prescriptions as $prescription)
                ...
                <div class="bg-[#f4f4f5] rounded-2xl p-4 sm:p-5 relative">
                    <div class="flex justify-between items-start mb-4">
                        <div>
                            <h2 class="text-[17px] sm:text-lg font-bold text-black">{{ $prescription->medication->medication_name }}</h2>
                        ...
                        <a href="{{ route('pharmaceutical.medications.stock', ['patient' => $patient->id, 'item' => $medId]) }}" class="text-[#0b67c2] hover:text-blue-800 transition ml-2 flex-shrink-0">
                    ...
                    <div class="space-y-2">
                        <div class="flex justify-between items-center">
                            ...
                            <span class="text-gray-400 font-semibold text-sm sm:text-base">{{ $prescription->dose }} {{ $prescription->dose_unit }}</span>
                        </div>
                        <div class="flex justify-between items-center">
                            <span class="font-bold text-black text-sm sm:text-base">Stock actual:</span>
                            <span class="{{ $isLowStock ? 'text-red-500' : 'text-gray-400' }} font-semibold text-sm sm:text-base">{{ $stockCount }} pastillas</span>
                        </div>
                    </div>
                </div>
            @empty
