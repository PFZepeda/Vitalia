<div class="relative min-h-screen bg-white">
            ...
            <div class="flex flex-col gap-4">
                @forelse($patients as $patient)
                    ...
                    <div class="relative flex items-center gap-4 rounded-[16px] bg-[#f4f5f7] p-5 shadow-sm">
                        ...
                        <div class="absolute right-4 top-8 flex flex-col gap-2 items-end">
                            <a href="{{ route('doctor.patients.show', $patient) }}" class="text-[#0b67c2]">
                                <i class="fa-regular fa-pen-to-square text-[18px]"></i>
                            </a>
                            <button wire:click="openReportsModal({{ $patient->id }})" class="mt-2 bg-[#0b67c2] text-white text-[10px] font-bold px-3 py-1.5 rounded-lg hover:bg-blue-700 transition shadow-sm">
                                Lista de reportes
                            </button>
                        </div>
                    </div>
    ...
    @if($showReportsModal && $selectedPatient)
    <div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-in fade-in duration-300">
        <div class="bg-white rounded-[24px] w-full max-w-[360px] flex flex-col shadow-2xl animate-in zoom-in-95 duration-300">
            ...
            <div class="p-6 pb-2 flex justify-between items-start">
                <div>
                    ...
                    <p class="text-[12px] text-slate-400 font-medium mt-1 leading-tight">
                        Reportes semanales de {{ $selectedPatient->name }}<br>
            ...
            <div class="p-6 pt-4 max-h-[400px] overflow-y-auto">
                ...
                @forelse($reports as $reportDate)
                    <div class="flex items-center justify-between py-4 border-b border-slate-50 last:border-none">
                        <span class="text-[14px] font-bold text-slate-700">{{ $reportDate->format('d/m/Y') }}</span>
                        <a href="{{ route('doctor.reports.weekly', ['patient' => $selectedPatient->id, 'date' => $reportDate->format('Y-m-d')]) }}" class="bg-[#0b67c2] text-white text-[12px] font-bold px-4 py-2 rounded-xl hover:bg-blue-700 transition">
